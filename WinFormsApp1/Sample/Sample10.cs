...
namespace WinFormsApp1
{
    ...
    class Sample10
    {
        ...
        {
            Debug.WriteLine("Build Vocabulary.");
            ...
            Debug.WriteLine("Network Initilizing.");
            ...
            for (int i = 0; i < jump * N_EPOCH; i++)
            {
                ...
                Debug.WriteLine("[{0}/{1}] Loss: {2}", i + 1, jump, sumLoss);
                ...
                if ((i + 1) % jump == 0)
                {
                    ...
                    Debug.WriteLine("validation perplexity: {0}", Evaluate(model, validData));

                    if (epoch >= 6)
                    {
                        sgd.LearningRate /= 1.2f;
                        Debug.WriteLine("learning rate =" + sgd.LearningRate);
                    }
                }
            }

            Debug.WriteLine("test start");
            Debug.WriteLine("test perplexity:" + Evaluate(model, testData));
        }
        ...
        static Real Evaluate(FunctionStack<Real> model, int[] dataset)
        {
            ...
            //calc perplexity
            return (float)Math.Exp(totalLoss / (totalLossCount - 1));
        }
    }
}
